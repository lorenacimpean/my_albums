...
import com.example.myalbums.repo.MissingPermissionsError
...
class ContactDetailsViewModel(
    ...
    val output: Output by lazy {
        ...
        val onLocationResult = input.clickLocation.rx.flatMap {
            return@flatMap locationRepo.getCurrentLocation().map {
                LocationResult(location = it, error = null)
            }.onErrorReturn {
                //todo check if it is Missing...
                LocationResult(location = null, error = it as MissingPermissionsError)
            }
                //TODO if share is needed here
                .toObservable().share()
        }
